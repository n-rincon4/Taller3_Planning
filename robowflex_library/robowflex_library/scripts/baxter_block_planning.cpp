/* Author: Carlos Quintero Pena */

#include <robowflex_library/builder.h>
#include <robowflex_library/geometry.h>
#include <robowflex_library/io/broadcaster.h>
#include <robowflex_library/io/visualization.h>
#include <robowflex_library/log.h>
#include <robowflex_library/scene.h>
#include <robowflex_library/trajectory.h>
#include <robowflex_library/util.h>
#include <robowflex_library/planning.h>

using namespace robowflex;

static const std::string GROUP = "right_arm";
static const std::string OBJECT = "Cube3";
static const std::string END_EFFECTOR = "right_gripper_base";
static const std::string SCENE_FILE = "package://robowflex_library/yaml/blocks_table.yml";
static const std::string OMPL_PLANNER = "package://robowflex_resources/baxter/config/ompl_planning.yaml";

int main(int argc, char **argv)
{
    // Startup ROS
    ROS ros(argc, argv);

    // Create a baxter robot.
    auto baxter = std::make_shared<Robot>("baxter");
    baxter->initializeFromYAML("package://robowflex_resources/baxter.yml");
    baxter->loadKinematics(GROUP);

    // Create an RViz visualization helper. Publishes all topics and parameter under `/robowflex` by default.
    IO::RVIZHelper rviz(baxter);
    IO::RobotBroadcaster bc(baxter);
    bc.start();

    RBX_INFO("RViz Initialized! Press enter to continue (after your RViz is setup)...");
    std::cin.get();

    // Load a scene from a YAML file.
    auto scene = std::make_shared<Scene>(baxter);
    scene->fromYAMLFile(SCENE_FILE);
    scene->getCurrentState() = *baxter->getScratchState();
    
    // Move the robot down.
    auto robot_offset = TF::createPoseQ(Eigen::Vector3d{0.18, 0.2, -0.75}, Eigen::Quaterniond{0.707, 0, 0, -0.707});//-90 z
    scene->moveAllObjectsGlobal(robot_offset);

    // Visualize the scene in RViz.
    rviz.updateScene(scene);
    
    // Define goal
    auto object_pose = scene->getObjectPose(OBJECT);
    auto right_grasp_offset = TF::createPoseQ(Eigen::Vector3d{0, 0, 0.18}, Eigen::Quaterniond{0, 0, -1, 0});
    RobotPose pose;
    pose = TF::identity();
    pose.translate(object_pose.translation());
    pose.translate(right_grasp_offset.translation());
    pose.rotate(right_grasp_offset.linear());
    
    rviz.addTransformMarker("goal", "map", pose);
    rviz.updateMarkers();
    
    // Create IK query.
    const auto query = Robot::IKQuery(GROUP, {pose}, {END_EFFECTOR}, constants::ik_tolerance,
                                          constants::ik_vec_tolerance, scene);
                                              
    // Set goal configuration from IK query.
    if (not baxter->setFromIK(query))
    {
        RBX_ERROR("IK solution not found");
        return 0;
    }
    
    RBX_INFO("Scene and Goal displayed! Press enter to plan...");
    std::cin.get();

    // Create the default planner for the baxter.
    auto planner = std::make_shared<OMPL::OMPLPipelinePlanner>(baxter);
    OMPL::Settings settings;
    planner->initialize(OMPL_PLANNER,  // planner config
                        settings);

    // Create a motion planning request.
    MotionRequestBuilder request(planner, GROUP);
    request.useSceneStateAsStart(scene);
    request.setGoalConfiguration(baxter->getScratchState());    

    // Do motion planning!
    planning_interface::MotionPlanResponse res = planner->plan(scene, request.getRequest());
    if (res.error_code_.val != moveit_msgs::MoveItErrorCodes::SUCCESS)
        return 1;

    // Publish the trajectory to a topic to display in RViz.
    rviz.updateTrajectory(res);

    // Create a trajectory object for better manipulation.
    auto trajectory = std::make_shared<Trajectory>(res.trajectory_);

    // Output path to a file for visualization.
    trajectory->toYAMLFile("baxter_block.yml");

    RBX_INFO("Press enter to remove goal and scene.");
    std::cin.get();

    // Clean up RViz.
    rviz.removeMarker("goal");
    rviz.updateMarkers();
    rviz.removeScene();

    RBX_INFO("Press enter to exit.");
    std::cin.get();

    return 0;
}
